    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          accent
          label="Entries"
          value={s?.rangeEntries ?? "—"}
          trend={t?.entries}
          hint={`last ${label.toLowerCase()}`}
        />
        <StatCard
          label="Raffles"
          value={s?.rangeRaffles ?? "—"}
          trend={t?.raffles}
          hint={`last ${label.toLowerCase()}`}
        />
        <StatCard
          label="Unique Players"
          value={s?.uniqueParticipants ?? "—"}
          hint="distinct entrants"
        />
        <StatCard
          label="Winners"
          value={s?.rangeWinners ?? "—"}
          trend={t?.winners}
          hint={`last ${label.toLowerCase()}`}
        />
      </div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <SectionTitle
            action={
              <span className="rounded-full border border-white/[0.08] px-2 py-0.5 text-[11px] text-kos-muted">
                Live refresh
              </span>
            }
          >
        ...
        <Card>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Link href={`/${org}/raffles`} className="kos-btn w-full">
                Raffles
              </Link>
              <Link href={`/${org}/tasks`} className="kos-btn-primary w-full">
                Tasks
              </Link>
            </div>
          </div>
            ...
            {data.live.map((r) => (
              ...
              >
                ...
                <RaffleQuickActions
                  ...
                  editHref={
                    canEdit ? `/${org}/raffles/${r.id}?edit=1` : undefined
                  }
                />
              </div>
            ))}
