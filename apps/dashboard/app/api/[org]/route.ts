...
/** Update org branding (name, logo, banner, description). Slug is immutable. */
export async function PATCH(req: Request, { params }: { params: { org: string } }) {
  try {
    const { org, user } = await requireOrgAccess(params.org, PERMISSIONS.BRANDING_EDIT);
    ...
    await logAudit(org.id, user.id, "ORG_BRANDING_UPDATE", { targetType: "organization", targetId: org.id });
    return NextResponse.json({
      org: { slug: updated.slug, name: updated.name, logoUrl: updated.logoUrl },
    });
  } catch (err) {
