...
export default async function MeProfilePage() {
  ...
  const [x, entryCount, winCount, orgCount] = await Promise.all([
    ...
    prisma.organizationMember.count({ where: { userId: user.id, status: "ACTIVE" } }),
  ]);
  ...
  return (
    <>
      ...
      <div className="mb-4 grid grid-cols-3 gap-3">
        <Stat label="Raffles entered" value={entryCount} />
        <Stat label="Wins" value={winCount} />
        <Stat label="Teams" value={orgCount} />
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="kos-card p-5">
          <div className="flex items-center gap-3">
            <div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-xl bg-[#5865F2]/20 text-lg">
              {user.avatarUrl ? (
            ...
            <div className="flex-1">
              ...
              <div className="text-sm text-kos-muted">
                ...
                <span className="ml-2 text-[11px] text-kos-muted/70">@{user.username}</span>
              </div>
            </div>
        ...
        <XConnectCard
          linked={
            x
              ? { handle: x.handle, avatar: meta.avatar ?? null, since: fmtDate(x.createdAt) }
              : null
          }
        />
        ...
        <Card className="opacity-50">
          <div className="flex items-center justify-between">
            ...
            <button disabled className="kos-btn cursor-not-allowed text-xs opacity-60">
              Soon
            </button>
          </div>
        </Card>
...
function Stat({ label, value }: { label: string; value: number }) {
  return (
    <div className="kos-card p-4">
      <div className="text-xs uppercase tracking-wide text-kos-muted">{label}</div>
