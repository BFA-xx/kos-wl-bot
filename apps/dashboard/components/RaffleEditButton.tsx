...
export function RaffleEditButton({ raffle }: { raffle: EditableRaffle }) {
  ...
  if (!canEdit || raffle.status === "ENDED" || raffle.status === "CANCELLED") return null;
  return (
    <>
      <button className="kos-btn" onClick={() => setOpen(true)}>
        Edit raffle
      </button>
      {open ? <EditModal raffle={raffle} onClose={() => setOpen(false)} /> : null}
    </>
  );
}

function EditModal({ raffle, onClose }: { raffle: EditableRaffle; onClose: () => void }) {
  const { slug } = useOrg();
  const router = useRouter();
  const [meta, setMeta] = useState<{ channels: { id: string; name: string }[]; roles: { id: string; name: string }[]; hasBotToken: boolean } | null>(null);
  ...
  const [announceChannelId, setAnnounceChannelId] = useState(raffle.announceChannelId ?? "");
  const [proofChannelId, setProofChannelId] = useState(raffle.proofChannelId ?? "");
  const [roleIds, setRoleIds] = useState<string[]>(raffle.roles.map((r) => r.roleId));
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 backdrop-blur-sm">
      <div className="flex min-h-full items-start justify-center p-4">
        <form onSubmit={save} className="my-6 w-full max-w-lg rounded-2xl border border-kos-border bg-kos-bg p-6">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold">Edit raffle #{raffle.id}</h2>
            <button type="button" onClick={onClose} className="text-kos-muted hover:text-kos-fg">
              <IconClose />
            </button>
          </div>

          {error ? <p className="mb-3 rounded-xl border border-red-500/20 bg-red-500/5 px-3 py-2 text-sm text-red-400">{error}</p> : null}

          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <F label="Project name"><input className="kos-input" value={projectName} onChange={(e) => setProjectName(e.target.value)} /></F>
              <F label="Title"><input className="kos-input" value={title} onChange={(e) => setTitle(e.target.value)} /></F>
            </div>
            <F label="Description"><textarea className="kos-input min-h-[60px]" value={description} onChange={(e) => setDescription(e.target.value)} /></F>
            <ImageDrop label="Banner image" value={bannerUrl} onChange={setBannerUrl} />
            <div className="grid grid-cols-2 gap-3">
              <F label="WL spots"><input type="number" min={1} className="kos-input" value={spots} onChange={(e) => setSpots(Number(e.target.value))} /></F>
              <F label="End"><input type="datetime-local" className="kos-input" value={endAt} onChange={(e) => setEndAt(e.target.value)} /></F>
            </div>
              ...
              <div className="space-y-2">
                {tasks.map((t, i) => (
                  <div key={i} className="flex gap-2">
                    <input className="kos-input" placeholder="Label" value={t.label} onChange={(e) => setTasks(tasks.map((x, j) => (j === i ? { ...x, label: e.target.value } : x)))} />
                    <input className="kos-input" placeholder="https://… (optional)" value={t.url} onChange={(e) => setTasks(tasks.map((x, j) => (j === i ? { ...x, url: e.target.value } : x)))} />
                    <button type="button" onClick={() => setTasks(tasks.filter((_, j) => j !== i))} className="shrink-0 px-2 text-kos-muted hover:text-red-400">✕</button>
                  </div>
                ))}
                {tasks.length < 10 ? <button type="button" onClick={() => setTasks([...tasks, { label: "", url: "" }])} className="kos-btn text-xs">+ Add task</button> : null}
              </div>
            </F>
            ...
            <div className="flex flex-wrap gap-4 text-sm">
              <label className="flex items-center gap-2"><input type="checkbox" checked={hideEntries} onChange={(e) => setHideEntries(e.target.checked)} /> Hide entry count</label>
              <label className="flex items-center gap-2"><input type="checkbox" checked={collectWallets} onChange={(e) => setCollectWallets(e.target.checked)} /> Collect wallets</label>
              <label className="flex items-center gap-2"><input type="checkbox" checked={requireWallet} onChange={(e) => setRequireWallet(e.target.checked)} /> Require wallet</label>
            </div>
          </div>

          <div className="mt-5 flex items-center justify-end gap-2">
            <button type="button" onClick={onClose} className="kos-btn">Cancel</button>
            <button type="submit" disabled={busy} className="kos-btn-primary">{busy ? "Saving…" : "Save changes"}</button>
          </div>
          <p className="mt-2 text-right text-[11px] text-kos-muted/70">If it's already posted, the bot updates the Discord message within a few seconds.</p>
        </form>
      </div>
    </div>,
    document.body,
