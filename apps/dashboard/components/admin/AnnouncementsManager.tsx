...
export function AnnouncementsManager({
  ...
  async function create(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/announcements", {
      ...
      body: JSON.stringify({
        ...
        body,
        level,
        organizationId: target === "all" ? null : target,
      }),
    });
    if (res.ok) {
  ...
  return (
    <div className="space-y-4">
      <form
        onSubmit={create}
        className="space-y-2 rounded-xl border border-kos-border bg-kos-panel/50 p-4"
      >
        <input
          className="kos-input"
          placeholder="Title"
        ...
        />
        <textarea
          className="kos-input min-h-[70px]"
          placeholder="Message"
          value={body}
        ...
        <div className="flex flex-wrap gap-2">
          <select
            className="kos-input sm:max-w-[140px]"
            value={level}
      ...
      <div className="space-y-2">
        {items.length === 0 ? (
          ...
          items.map((a) => (
            <div
              key={a.id}
              className="flex flex-col items-start justify-between gap-3 rounded-xl border border-kos-border bg-kos-panel/50 p-3 sm:flex-row"
            >
