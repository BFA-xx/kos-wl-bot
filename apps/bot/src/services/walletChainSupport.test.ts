...
test("Robinhood raffles never fall back to a Solana profile", () => {
  const solana = { chain: WalletChain.SOLANA, id: "sol" };
  const robinhood = { chain: WalletChain.ROBINHOOD, id: "rh" };
  assert.equal(
    selectConfiguredWallet(null, [solana], [WalletChain.ROBINHOOD]),
    null,
  );
  assert.equal(
    selectConfiguredWallet(
